...
describe("resolveLoginFailureFeedback", () => {
  ...
  it("explains missing auth credentials", () => {
    ...
    expect(feedback?.steps).toEqual([
      "Paste the token from zcusclaw dashboard --no-open or enter the configured password.",
      "If no token is configured, run zcusclaw doctor --generate-gateway-token on the gateway host.",
  ...
  it("explains rejected stale credentials", () => {
    ...
    expect(feedback?.steps).toEqual([
      "Run zcusclaw dashboard --no-open and open the fresh URL or paste its token.",
  ...
  it("preserves pairing request ids in the approval command", () => {
    ...
    expect(feedback?.steps).toEqual([
      "Run zcusclaw devices list on the Gateway host.",
      "Approve this request: zcusclaw devices approve req-123.",
  ...
  it("keeps generic browser WebSocket constructor failures on the network path", () => {
    ...
    expect(feedback?.steps).toEqual([
      "Confirm the Gateway is running with zcusclaw status or zcusclaw gateway run.",
      "Check the WebSocket URL and use wss:// when the Gateway is behind HTTPS/Tailscale Serve.",
      "Reopen the dashboard with zcusclaw dashboard --no-open to recopy the current URL and auth details.",
    ]);
  });
  ...
  it("explains protocol mismatch without requiring a gateway protocol change", () => {
    ...
    expect(feedback?.steps).toEqual([
      "Reopen the served dashboard with zcusclaw dashboard so the UI and Gateway come from the same install.",
      "If using pnpm ui:dev, rebuild or restart the dev UI against the current checkout.",
      "Restart the Gateway after updating zcusclaw so it serves the current protocol.",
    ]);
  });

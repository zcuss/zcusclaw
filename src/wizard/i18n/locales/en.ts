...
export const en = {
  ...
  wizard: {
    ...
    channels: {
      ...
      configureDmPolicies: "Configure DM access rules now? (default: pairing / approval required)",
      deleteAccount: 'Delete {label} account "{account}"?',
      deleteConfig: "Delete config",
      dmAccessTitle: "{label} DM access: approve / allow / block",
      dmPolicy: "How should {label} handle DMs?",
      dmPolicyAllowlist:
        'Allowlist mode: set {policyKey}="allowlist" and store allowed sender IDs in {allowFromKey}.',
      dmPolicyAllowlistOption: "Allow selected senders only (allowlist)",
      dmPolicyApprove: "Approve pairing requests with: {command}",
      dmPolicyDefault:
        "Default approve flow: unknown DMs receive a pairing code and stay blocked until approved.",
      dmPolicyDisabledOption: "Block all DMs (ignore inbound DMs)",
      dmPolicyMultiUser:
        'Multi-user DMs: run: {command} (or "per-account-channel-peer" for multi-account channels) to isolate sessions.',
      dmPolicyOpen: 'Allow-all mode: set {policyKey}="open" and include "*" in {allowFromKey}.',
      dmPolicyOpenOption: "Allow all senders (public inbound)",
      dmPolicyPairing: "Approve first: pairing code required (recommended)",
      docs: "Docs: {link}",
